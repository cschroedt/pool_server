# Version 5.53 - 26_03_10
# Port 2234
# Frostschutz WPumpe - int. Temp um 1 Grad nach unten korrigiert
# Sommer-/Winterzeit
...
ATempKoeff0=286.47
ATempKoeff1=-382.79
ATempOffset=1.0	# Außentemp über Pico

PFfrei=49.9		# Pumpenleistung Filer frei
PFsaett=29.8	# Pumpenleistung Filter gesättigt
...
try:
    datei=open("ATempKoeff0.txt",'r')
    ATempKoeff0=datei.read()
    datei.close
except:
    print("Fehler lesen ATempKoff0")
try:
    datei=open("ATempKoeff1.txt",'r')
    ATempKoeff1=datei.read()
    datei.close
except:
    print("Fehler lesen ATempKoff1")
try:
    datei=open("ATempOffset.txt",'r')
    ATempOffset=datei.read()
    datei.close
except:
    print("Fehler lesen ATempOffset")
    
try:
    datei=open("PFfrei.txt",'r')
    PFfrei=datei.read()
    datei.close
except:
    print("Fehler lesen PFfrei")
try:
    datei=open("PFsaett.txt",'r')
    PFsaett=datei.read()
    datei.close
except:
    print("Fehler lesen PFsaett")
    
try:
    datei=open("PStart.txt",'r')
    PStart=datei.read()
    datei.close
except:
    print("Fehler lesen PStart")
try:
    datei=open("PDauer.txt",'r')
    PDauer=datei.read()
    datei.close
except:
    print("Fehler lesen PDauer")
...
def tick(timer):  # hier ist die Zeitabfrage
    ...
    temperatur = TempKoeff0 + spannung*TempKoeff1 -1
...
while True:
    ...
    '''
        ...
        UDPServer.sendto(backEncoded,address)
    elif messageDecoded.find('27')>-1 and messageDecoded.find('27') < 2: # setze ATempOffset
        try:
            ATempOffset=float(messageDecoded[3:])
            datei=open("ATempOffset",'w')
            datei.write(ATempOffset)
            datei.close()
        except:
            print("Except 27")
    elif messageDecoded.find("28")>-1 and messageDecoded.find('28') < 2: # setze ATempKoeff0
        try:
            ATempKoeff0=float(messageDecoded[3:])
            datei=open("ATempKoeff0",'w')
            datei.write(ATempKoeff0)
            datei.close()
        except:
            print("Except 28")
    elif messageDecoded.find("29")>-1 and messageDecoded.find('29') < 2: # setze ATempKoeff1
        try:
            TempKoeff1=float(messageDecoded[3:])
            datei=open("ATempKoeff1",'w')
            datei.write(ATempKoeff1)
            datei.close()
        except:
            print("Except 29")
    ...
    elif (messageDecoded.find("60")>-1): # Startzeit Pumpe setzen (Stunde)
        #message,address=UDPServer.recvfrom(bufferSize)
        #messageDecoded=message.decode('utf-8')
        pStart=int(messageDecoded[3:])
        datei=open("pStart.txt",'w')
        datei.write(pStart)
        datei.close()
    elif (messageDecoded == "61"): # Startzeit Pumpe lesen
        backEncoded=str(pStart).encode('utf-8')
        UDPServer.sendto(backEncoded,address)
    elif (messageDecoded.find("62")>-1): # Laufdauer Pumpe setzen (Stunden)
        #message,address=UDPServer.recvfrom(bufferSize)
        #messageDecoded=message.decode('utf-8')
        pDauer=int(messageDecoded[3:])        
        datei=open("pDauer.txt",'w')
        datei.write(pDauer)
        datei.close()
